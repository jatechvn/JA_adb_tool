...
class MirrorReviewLogic extends AppLogic {
  MirrorReviewLogic() : super(initialize: false);
  @override
  String? get selectedDevice => 'test-device';
  @override
...
void main() {
  for (final locale in ['en', 'vi', 'zh']) {
    for (final light in [false, true]) {
      testWidgets(
        'Mirror 1280x800 $locale light=$light with connected device',
        (tester) async {
          ...
          final logic = MirrorReviewLogic();
          final theme = ThemeProvider();
          if (light) theme.toggleTheme();
            ...
            MultiProvider(
              providers: [
                ...
                ChangeNotifierProvider(
                  create: (_) => LanguageProvider.forTesting(locale),
                ),
              ],
          ...
          for (final label in [
            ...
            LanguageProvider.forTesting(locale).tr('scrcpy_options'),
            'Collapse Sidebar',
            'Expand Sidebar',
          ]) {
